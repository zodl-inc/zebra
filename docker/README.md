...
`Dockerfile.deterministic`, `build.sh`, this README, and the `.gitignore`
additions originate from **Anton Livaja (@antonleviathan) / Distrust** in the
upstream pull request
[ZcashFoundation/zebra#10491](https://github.com/ZcashFoundation/zebra/pull/10491)
("feat: introduce deterministic builds using stagex"). `compat.sh` and
`update-stagex.sh` originate from the earlier
[ZcashFoundation/zebra#10068](https://github.com/ZcashFoundation/zebra/pull/10068).
They were ported into the `zodl-inc/zebra` fork on the `feat/stagex-reproducible`
branch (Linear COR-1293) and lightly adapted to this fork's tree.

### Requirements

The deterministic build needs **Docker 26+** with the **containerd image store**
enabled (so `--output type=oci,rewrite-timestamp=true` produces reproducible
image timestamps) and **buildx 0.13+**. The `build.sh` script runs `compat.sh`
as a preflight and will tell you what is missing. To enable the containerd image
store, set `"features": { "containerd-snapshotter": true }` in the Docker daemon
config and restart the daemon (or enable it in Docker Desktop settings).

### Toolchain version note

The StageX `pallet-rust` base is pinned to **1.94.0** (by SHA-256 digest) and is
the toolchain the deterministic build actually compiles with. This is newer than
the standard `Dockerfile`'s `RUST_VERSION=1.91.0` and the workspace MSRV
(`rust-version = "1.85.1"` in `Cargo.toml`), so it satisfies both. The
`ARG RUST_VERSION` in `Dockerfile.deterministic` is documentary only — the real
version comes from the pinned StageX image, not the ARG.
...
### Bumping StageX base image digests

StageX bases are pinned by digest. To advance them to the latest signed
release, run from the `docker/` directory:

```sh
cd docker && ./update-stagex.sh
```

Review the resulting diff and only advance to digests that StageX has
published as independently reproduced and co-signed.
...
`docker/stagex-reference/` documents the exact pinned StageX bases and includes
the upstream `pallet/rust` Containerfile + package manifest, so the reproduction
inputs are legible from inside this repo without cloning all of StageX. The full
distribution lives at <https://codeberg.org/stagex/stagex>.
